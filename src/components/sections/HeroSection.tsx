import { useCallback, useEffect, useRef, useState } from "react";
import { useLenis } from "@/motion/LenisProvider";
import "./HeroSection.css";

/* Locked hysteresis B–H mark (viewBox -100 -100 200 200). Two branches + closed fill. */
const UP =
  "M 75 -55.98 C 74.65 -55.97, 73.61 -55.97, 72.92 -55.97 C 72.22 -55.97, 71.53 -55.97, 70.83 -55.97 C 70.14 -55.96, 69.44 -55.96, 68.75 -55.96 C 68.06 -55.96, 67.36 -55.95, 66.67 -55.95 C 65.97 -55.95, 65.28 -55.94, 64.58 -55.94 C 63.89 -55.94, 63.19 -55.93, 62.5 -55.93 C 61.81 -55.92, 61.11 -55.92, 60.42 -55.91 C 59.72 -55.91, 59.03 -55.9, 58.33 -55.9 C 57.64 -55.89, 56.94 -55.88, 56.25 -55.88 C 55.56 -55.87, 54.86 -55.86, 54.17 -55.85 C 53.47 -55.84, 52.78 -55.83, 52.08 -55.82 C 51.39 -55.81, 50.69 -55.8, 50 -55.79 C 49.31 -55.77, 48.61 -55.76, 47.92 -55.74 C 47.22 -55.73, 46.53 -55.71, 45.83 -55.69 C 45.14 -55.68, 44.44 -55.66, 43.75 -55.63 C 43.06 -55.61, 42.36 -55.59, 41.67 -55.56 C 40.97 -55.53, 40.28 -55.51, 39.58 -55.47 C 38.89 -55.44, 38.19 -55.41, 37.5 -55.37 C 36.81 -55.33, 36.11 -55.29, 35.42 -55.24 C 34.72 -55.2, 34.03 -55.15, 33.33 -55.1 C 32.64 -55.04, 31.94 -54.98, 31.25 -54.92 C 30.56 -54.85, 29.86 -54.78, 29.17 -54.71 C 28.47 -54.63, 27.78 -54.55, 27.08 -54.45 C 26.39 -54.36, 25.69 -54.26, 25 -54.15 C 24.31 -54.04, 23.61 -53.92, 22.92 -53.79 C 22.22 -53.66, 21.53 -53.52, 20.83 -53.36 C 20.14 -53.21, 19.44 -53.04, 18.75 -52.85 C 18.06 -52.67, 17.36 -52.47, 16.67 -52.25 C 15.97 -52.03, 15.28 -51.79, 14.58 -51.53 C 13.89 -51.27, 13.19 -51, 12.5 -50.69 C 11.81 -50.38, 11.11 -50.05, 10.42 -49.69 C 9.72 -49.33, 9.03 -48.95, 8.33 -48.52 C 7.64 -48.1, 6.94 -47.65, 6.25 -47.16 C 5.56 -46.66, 4.86 -46.14, 4.17 -45.56 C 3.47 -44.99, 2.78 -44.38, 2.08 -43.72 C 1.39 -43.06, 0.69 -42.35, 0 -41.59 C -0.69 -40.83, -1.39 -40.03, -2.08 -39.16 C -2.78 -38.3, -3.47 -37.38, -4.17 -36.4 C -4.86 -35.42, -5.56 -34.39, -6.25 -33.3 C -6.94 -32.21, -7.64 -31.05, -8.33 -29.84 C -9.03 -28.63, -9.72 -27.36, -10.42 -26.04 C -11.11 -24.71, -11.81 -23.33, -12.5 -21.9 C -13.19 -20.47, -13.89 -18.98, -14.58 -17.46 C -15.28 -15.93, -15.97 -14.36, -16.67 -12.76 C -17.36 -11.16, -18.06 -9.51, -18.75 -7.86 C -19.44 -6.21, -20.14 -4.52, -20.83 -2.84 C -21.53 -1.16, -22.22 0.55, -22.92 2.23 C -23.61 3.91, -24.31 5.61, -25 7.26 C -25.69 8.92, -26.39 10.57, -27.08 12.18 C -27.78 13.79, -28.47 15.37, -29.17 16.91 C -29.86 18.44, -30.56 19.94, -31.25 21.38 C -31.94 22.82, -32.64 24.22, -33.33 25.56 C -34.03 26.9, -34.72 28.18, -35.42 29.4 C -36.11 30.63, -36.81 31.8, -37.5 32.9 C -38.19 34.01, -38.89 35.06, -39.58 36.05 C -40.28 37.04, -40.97 37.97, -41.67 38.85 C -42.36 39.73, -43.06 40.55, -43.75 41.32 C -44.44 42.09, -45.14 42.81, -45.83 43.48 C -46.53 44.15, -47.22 44.77, -47.92 45.36 C -48.61 45.94, -49.31 46.47, -50 46.98 C -50.69 47.48, -51.39 47.94, -52.08 48.37 C -52.78 48.8, -53.47 49.19, -54.17 49.56 C -54.86 49.93, -55.56 50.26, -56.25 50.58 C -56.94 50.89, -57.64 51.17, -58.33 51.44 C -59.03 51.7, -59.72 51.94, -60.42 52.17 C -61.11 52.39, -61.81 52.6, -62.5 52.79 C -63.19 52.97, -63.89 53.15, -64.58 53.31 C -65.28 53.46, -65.97 53.61, -66.67 53.74 C -67.36 53.88, -68.06 54, -68.75 54.11 C -69.44 54.22, -70.14 54.32, -70.83 54.42 C -71.53 54.51, -72.22 54.6, -72.92 54.68 C -73.61 54.76, -74.65 54.86, -75 54.89";
const DOWN =
  "M -75 55.98 C -74.65 55.97, -73.61 55.97, -72.92 55.97 C -72.22 55.97, -71.53 55.97, -70.83 55.97 C -70.14 55.96, -69.44 55.96, -68.75 55.96 C -68.06 55.96, -67.36 55.95, -66.67 55.95 C -65.97 55.95, -65.28 55.94, -64.58 55.94 C -63.89 55.94, -63.19 55.93, -62.5 55.93 C -61.81 55.92, -61.11 55.92, -60.42 55.91 C -59.72 55.91, -59.03 55.9, -58.33 55.9 C -57.64 55.89, -56.94 55.88, -56.25 55.88 C -55.56 55.87, -54.86 55.86, -54.17 55.85 C -53.47 55.84, -52.78 55.83, -52.08 55.82 C -51.39 55.81, -50.69 55.8, -50 55.79 C -49.31 55.77, -48.61 55.76, -47.92 55.74 C -47.22 55.73, -46.53 55.71, -45.83 55.69 C -45.14 55.68, -44.44 55.66, -43.75 55.63 C -43.06 55.61, -42.36 55.59, -41.67 55.56 C -40.97 55.53, -40.28 55.51, -39.58 55.47 C -38.89 55.44, -38.19 55.41, -37.5 55.37 C -36.81 55.33, -36.11 55.29, -35.42 55.24 C -34.72 55.2, -34.03 55.15, -33.33 55.1 C -32.64 55.04, -31.94 54.98, -31.25 54.92 C -30.56 54.85, -29.86 54.78, -29.17 54.71 C -28.47 54.63, -27.78 54.55, -27.08 54.45 C -26.39 54.36, -25.69 54.26, -25 54.15 C -24.31 54.04, -23.61 53.92, -22.92 53.79 C -22.22 53.66, -21.53 53.52, -20.83 53.36 C -20.14 53.21, -19.44 53.04, -18.75 52.85 C -18.06 52.67, -17.36 52.47, -16.67 52.25 C -15.97 52.03, -15.28 51.79, -14.58 51.53 C -13.89 51.27, -13.19 51, -12.5 50.69 C -11.81 50.38, -11.11 50.05, -10.42 49.69 C -9.72 49.33, -9.03 48.95, -8.33 48.52 C -7.64 48.1, -6.94 47.65, -6.25 47.16 C -5.56 46.66, -4.86 46.14, -4.17 45.56 C -3.47 44.99, -2.78 44.38, -2.08 43.72 C -1.39 43.06, -0.69 42.35, 0 41.59 C 0.69 40.83, 1.39 40.03, 2.08 39.16 C 2.78 38.3, 3.47 37.38, 4.17 36.4 C 4.86 35.42, 5.56 34.39, 6.25 33.3 C 6.94 32.21, 7.64 31.05, 8.33 29.84 C 9.03 28.63, 9.72 27.36, 10.42 26.04 C 11.11 24.71, 11.81 23.33, 12.5 21.9 C 13.19 20.47, 13.89 18.98, 14.58 17.46 C 15.28 15.93, 15.97 14.36, 16.67 12.76 C 17.36 11.16, 18.06 9.51, 18.75 7.86 C 19.44 6.21, 20.14 4.52, 20.83 2.84 C 21.53 1.16, 22.22 -0.55, 22.92 -2.23 C 23.61 -3.91, 24.31 -5.61, 25 -7.26 C 25.69 -8.92, 26.39 -10.57, 27.08 -12.18 C 27.78 -13.79, 28.47 -15.37, 29.17 -16.91 C 29.86 -18.44, 30.56 -19.94, 31.25 -21.38 C 31.94 -22.82, 32.64 -24.22, 33.33 -25.56 C 34.03 -26.9, 34.72 -28.18, 35.42 -29.4 C 36.11 -30.63, 36.81 -31.8, 37.5 -32.9 C 38.19 -34.01, 38.89 -35.06, 39.58 -36.05 C 40.28 -37.04, 40.97 -37.97, 41.67 -38.85 C 42.36 -39.73, 43.06 -40.55, 43.75 -41.32 C 44.44 -42.09, 45.14 -42.81, 45.83 -43.48 C 46.53 -44.15, 47.22 -44.77, 47.92 -45.36 C 48.61 -45.94, 49.31 -46.47, 50 -46.98 C 50.69 -47.48, 51.39 -47.94, 52.08 -48.37 C 52.78 -48.8, 53.47 -49.19, 54.17 -49.56 C 54.86 -49.93, 55.56 -50.26, 56.25 -50.58 C 56.94 -50.89, 57.64 -51.17, 58.33 -51.44 C 59.03 -51.7, 59.72 -51.94, 60.42 -52.17 C 61.11 -52.39, 61.81 -52.6, 62.5 -52.79 C 63.19 -52.97, 63.89 -53.15, 64.58 -53.31 C 65.28 -53.46, 65.97 -53.61, 66.67 -53.74 C 67.36 -53.88, 68.06 -54, 68.75 -54.11 C 69.44 -54.22, 70.14 -54.32, 70.83 -54.42 C 71.53 -54.51, 72.22 -54.6, 72.92 -54.68 C 73.61 -54.76, 74.65 -54.86, 75 -54.89";
const FILL = `${UP} L -75 55.98 ${DOWN.replace("M -75 55.98", "")} Z`;
const SEEN_KEY = "ff-hero-splash-v1";

export default function HeroSection() {
  const lenis = useLenis();

  // Decide synchronously (before paint) whether to play the splash — avoids any flash.
  const [splash] = useState(() => {
    if (typeof window === "undefined") return false;
    try {
      const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      return !reduce && sessionStorage.getItem(SEEN_KEY) !== "1";
    } catch {
      return true;
    }
  });

  const sectionRef = useRef<HTMLElement>(null);
  const stageRef = useRef<HTMLDivElement>(null);
  const upRef = useRef<SVGPathElement>(null);
  const downRef = useRef<SVGPathElement>(null);
  const fillRef = useRef<SVGPathElement>(null);
  const cometUpRef = useRef<SVGCircleElement>(null);
  const cometDnRef = useRef<SVGCircleElement>(null);
  const opRef = useRef<SVGCircleElement>(null);
  const opGlowRef = useRef<SVGCircleElement>(null);
  const gatePosRef = useRef<SVGLineElement>(null);
  const gateNegRef = useRef<SVGLineElement>(null);
  const tHRef = useRef<HTMLElement>(null);
  const tBRef = useRef<HTMLElement>(null);
  const tStateRef = useRef<HTMLSpanElement>(null);

  const scrollToSection = useCallback(
    (id: string) => {
      if (lenis) {
        lenis.scrollTo(`#${id}`, { offset: -84 });
        return;
      }
      document.getElementById(id)?.scrollIntoView({ behavior: "smooth" });
    },
    [lenis]
  );

  useEffect(() => {
    const section = sectionRef.current;
    const stage = stageRef.current;
    const up = upRef.current;
    const down = downRef.current;
    const fill = fillRef.current;
    const cometUp = cometUpRef.current;
    const cometDn = cometDnRef.current;
    const op = opRef.current;
    const opGlow = opGlowRef.current;
    const gatePos = gatePosRef.current;
    const gateNeg = gateNegRef.current;
    const tH = tHRef.current;
    const tB = tBRef.current;
    const tState = tStateRef.current;
    if (!section || !stage || !up || !down || !fill || !cometUp || !cometDn || !op || !opGlow) return;

    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const coarse = window.matchMedia("(pointer: coarse)").matches;

    // ----- geometry -----
    const lenUp = up.getTotalLength();
    const lenDn = down.getTotalLength();
    const Xsat = 75;
    const Hc = 32;
    const Hmax = 88;
    const Bnorm = 55.98;

    type Pt = { x: number; y: number };
    const buildSamples = (path: SVGPathElement, len: number): Pt[] => {
      const N = 240;
      const arr: Pt[] = new Array(N + 1);
      for (let i = 0; i <= N; i++) {
        const pt = path.getPointAtLength((i / N) * len);
        arr[i] = { x: pt.x, y: pt.y };
      }
      return arr;
    };
    const sampUp = buildSamples(up, lenUp);
    const sampDn = buildSamples(down, lenDn);

    const pointAtX = (s: Pt[], targetX: number): Pt => {
      const a = s[0];
      const b = s[s.length - 1];
      const asc = b.x >= a.x;
      const tx = Math.max(Math.min(targetX, Math.max(a.x, b.x)), Math.min(a.x, b.x));
      let lo = 0;
      let hi = s.length - 1;
      while (hi - lo > 1) {
        const mid = (lo + hi) >> 1;
        const mx = s[mid].x;
        if ((asc && mx < tx) || (!asc && mx > tx)) lo = mid;
        else hi = mid;
      }
      const p0 = s[lo];
      const p1 = s[hi];
      const span = p1.x - p0.x;
      const t = Math.abs(span) < 1e-6 ? 0 : (tx - p0.x) / span;
      return { x: p0.x + (p1.x - p0.x) * t, y: p0.y + (p1.y - p0.y) * t };
    };
    const HtoX = (H: number) => (H / Hmax) * Xsat;
    const pointFor = (H: number, br: string) => pointAtX(br === "up" ? sampUp : sampDn, HtoX(H));
    const Bof = (p: Pt) => -p.y / Bnorm;

    // ----- state -----
    let branch = "up";
    let dispH = 0;
    let targetH = 0;
    let snap = 0;
    let snapDir = 1;
    let rafId = 0;
    let live = false;
    let drawStart: number | null = null;

    const fmt = (v: number) => {
      const s = v.toFixed(2);
      return v >= 0 && s[0] !== "-" ? `+${s}` : s;
    };
    const placeOp = (p: Pt) => {
      op.setAttribute("cx", p.x.toFixed(2));
      op.setAttribute("cy", p.y.toFixed(2));
      opGlow.setAttribute("cx", p.x.toFixed(2));
      opGlow.setAttribute("cy", p.y.toFixed(2));
    };
    const setText = (p: Pt) => {
      if (tH) tH.textContent = fmt(dispH / Hmax);
      if (tB) tB.textContent = fmt(Bof(p));
      let label: string;
      let locked = "0";
      const a = Math.abs(dispH);
      if (a < 4) {
        label = branch === "up" ? "RETENTIVE · +Br" : "RETENTIVE · −Br";
        locked = "1";
      } else if (a > Hmax - 14) {
        label = dispH > 0 ? "SATURATED · +" : "SATURATED · −";
      } else if (a > Hc - 8 && a < Hc + 12) {
        label = "COERCIVE · resisting";
      } else {
        label = branch === "up" ? "HOLDING · upper" : "HOLDING · lower";
      }
      if (tState) {
        tState.textContent = label;
        tState.setAttribute("data-locked", locked);
      }
    };
    const flashGate = (which: string) => {
      const g = which === "pos" ? gatePos : gateNeg;
      if (!g) return;
      g.style.transition = "none";
      g.style.opacity = "0.9";
      requestAnimationFrame(() => {
        g.style.transition = "opacity 1s ease";
        g.style.opacity = "0";
      });
    };
    const commit = (to: string) => {
      snap = 1;
      snapDir = to === "up" ? 1 : -1;
      const el = to === "up" ? up : down;
      el.style.transition = "none";
      el.style.color = "var(--ff-led)";
      requestAnimationFrame(() => {
        el.style.transition = "color 1.1s ease";
        el.style.color = "";
      });
    };

    // ----- live magnetic-field loop -----
    const frame = () => {
      if (!live) return;
      dispH += (targetH - dispH) * 0.11;
      if (branch === "up" && dispH < -Hc) {
        branch = "down";
        flashGate("neg");
        commit("down");
      } else if (branch === "down" && dispH > Hc) {
        branch = "up";
        flashGate("pos");
        commit("up");
      }
      const effH = dispH + snap * snapDir * 7;
      snap *= 0.82;
      if (snap < 0.01) snap = 0;
      const p = pointFor(effH, branch);
      placeOp(p);
      const drive = Math.abs(dispH) / Hmax;
      opGlow.setAttribute("r", (9 + drive * 7).toFixed(2));
      op.setAttribute("r", (5.4 + drive * 1.3).toFixed(2));
      setText(p);
      rafId = requestAnimationFrame(frame);
    };
    const startLive = () => {
      if (!live && !coarse && !reduce) {
        live = true;
        rafId = requestAnimationFrame(frame);
      }
    };
    const pauseLive = () => {
      live = false;
      if (rafId) cancelAnimationFrame(rafId);
    };
    const settleRemanence = () => {
      branch = "up";
      dispH = 0;
      targetH = 0;
      snap = 0;
      const p = pointFor(0, "up");
      placeOp(p);
      setText(p);
    };

    // ----- pointer = magnetising force -----
    const onMove = (clientX: number) => {
      const r = stage.getBoundingClientRect();
      const nx = (clientX - r.left) / r.width;
      targetH = Math.max(-Hmax, Math.min(Hmax, (nx * 2 - 1) * Hmax));
    };
    const onPointerMove = (e: PointerEvent) => onMove(e.clientX);
    const onPointerDown = (e: PointerEvent) => onMove(e.clientX);
    const onPointerLeave = () => {
      targetH = 0;
    };
    const addPointer = () => {
      if (coarse || reduce) return;
      stage.addEventListener("pointermove", onPointerMove, { passive: true });
      stage.addEventListener("pointerdown", onPointerDown, { passive: true });
      stage.addEventListener("pointerleave", onPointerLeave);
    };

    // ----- entrance: slow draw (with comet) -> fill -> reveal -> live -----
    const easeInOut = (t: number) => (t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2);
    const DRAW_MS = 2600;
    const FILL_MS = 1100;
    const FILL_GAP = 140;

    const reveal = () => {
      section.classList.add("revealed", "lit");
      settleRemanence();
      addPointer();
      startLive();
    };

    const endDraw = () => {
      [cometUp, cometDn].forEach((c) => {
        c.style.transition = "opacity .5s ease";
        c.style.opacity = "0";
      });
      fill.animate(
        [{ fillOpacity: 0 }, { fillOpacity: 1 }],
        { duration: FILL_MS, delay: FILL_GAP, easing: "ease", fill: "forwards" }
      );
      window.setTimeout(reveal, FILL_GAP + FILL_MS);
    };

    const drawFrame = (now: number) => {
      if (drawStart === null) drawStart = now;
      const t = Math.min(1, (now - drawStart) / DRAW_MS);
      const p = easeInOut(t);
      up.style.strokeDashoffset = String(1 - p);
      down.style.strokeDashoffset = String(1 - p);
      const pu = up.getPointAtLength(p * lenUp);
      const pd = down.getPointAtLength(p * lenDn);
      cometUp.setAttribute("cx", pu.x.toFixed(2));
      cometUp.setAttribute("cy", pu.y.toFixed(2));
      cometDn.setAttribute("cx", pd.x.toFixed(2));
      cometDn.setAttribute("cy", pd.y.toFixed(2));
      const r = (2.6 + p * 1.7).toFixed(2);
      cometUp.setAttribute("r", r);
      cometDn.setAttribute("r", r);
      if (t < 1) {
        rafId = requestAnimationFrame(drawFrame);
      } else {
        endDraw();
      }
    };

    const finalize = () => {
      [up, down].forEach((s) => {
        s.style.strokeDasharray = "none";
        s.style.strokeDashoffset = "0";
      });
      fill.style.fillOpacity = "1";
      cometUp.style.opacity = "0";
      cometDn.style.opacity = "0";
      reveal();
    };

    let seenNow = false;
    try {
      seenNow = sessionStorage.getItem(SEEN_KEY) === "1";
    } catch {
      seenNow = false;
    }

    if (splash && !reduce && !seenNow) {
      try {
        sessionStorage.setItem(SEEN_KEY, "1");
      } catch {
        /* ignore */
      }
      // strokes already primed hidden via JSX inline style; start the draw next paint
      settleRemanence();
      cometUp.style.opacity = "1";
      cometDn.style.opacity = "1";
      rafId = requestAnimationFrame(() => {
        rafId = requestAnimationFrame(drawFrame);
      });
    } else {
      finalize();
    }

    // ----- perf: pause the live loop offscreen / hidden -----
    const onVis = () => {
      if (document.hidden) pauseLive();
      else if (section.classList.contains("revealed")) startLive();
    };
    document.addEventListener("visibilitychange", onVis);
    const io = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && section.classList.contains("revealed")) startLive();
        else pauseLive();
      },
      { threshold: 0.04 }
    );
    io.observe(stage);

    return () => {
      pauseLive();
      if (rafId) cancelAnimationFrame(rafId);
      stage.removeEventListener("pointermove", onPointerMove);
      stage.removeEventListener("pointerdown", onPointerDown);
      stage.removeEventListener("pointerleave", onPointerLeave);
      document.removeEventListener("visibilitychange", onVis);
      io.disconnect();
    };
  }, [splash]);

  return (
    <section
      ref={sectionRef}
      id="hero"
      className={`ffhero${splash ? " drawing" : " revealed lit"}`}
      aria-label="Layth Ayache — hero"
    >
      <div className="ff-grain" aria-hidden="true" />
      <div className="ff-vignette" aria-hidden="true" />

      <div className="ff-field">
        <div className="ff-kicker ff-rise" data-d="1">
          <span className="tick" aria-hidden="true" /> AI Systems &amp; Automation Engineer — Beirut
        </div>

        <div
          ref={stageRef}
          className="ff-markwrap"
          role="img"
          aria-label="Hysteresis B–H loop — the mark for Layth Ayache. An interactive diagram: move your cursor left and right to apply a magnetising force; the operating point resists and lags, then commits to the other branch once it passes the coercivity threshold."
        >
          <svg className="ff-mark" viewBox="-100 -100 200 200" aria-hidden="true">
            <line ref={gateNegRef} className="ff-gate" x1={-32} y1={-18} x2={-32} y2={18} />
            <line ref={gatePosRef} className="ff-gate" x1={32} y1={-18} x2={32} y2={18} />
            <path ref={fillRef} className="ff-fillpath" d={FILL} style={{ fillOpacity: splash ? 0 : 1 }} />
            <path
              ref={upRef}
              className="ff-branch"
              pathLength={1}
              d={UP}
              style={{ strokeDasharray: 1, strokeDashoffset: splash ? 1 : 0 }}
            />
            <path
              ref={downRef}
              className="ff-branch"
              pathLength={1}
              d={DOWN}
              style={{ strokeDasharray: 1, strokeDashoffset: splash ? 1 : 0 }}
            />
            <circle ref={cometUpRef} className="ff-comet" r={3} cx={75} cy={-55.98} />
            <circle ref={cometDnRef} className="ff-comet" r={3} cx={-75} cy={55.98} />
            <circle ref={opGlowRef} className="ff-opglow" r={11} cx={0} cy={-55.98} />
            <circle ref={opRef} className="ff-op" r={6} cx={0} cy={-55.98} />
          </svg>
        </div>

        <div className="ff-telemetry ff-rise" data-d="2" aria-hidden="true">
          <span>
            <span className="t-h">H</span> <b ref={tHRef}>+0.00</b>
          </span>
          <span className="sep" />
          <span>
            <span className="t-h">B</span> <b ref={tBRef}>+1.00</b>
          </span>
          <span className="sep" />
          <span ref={tStateRef} className="state" data-locked="1">
            RETENTIVE · +Br
          </span>
        </div>

        <h1 className="ff-rise" data-d="3">
          I build systems that <em>hold under pressure.</em>
        </h1>
        <p className="ff-sub ff-rise" data-d="3">
          AI, automation, and infrastructure for systems that can&rsquo;t afford to fail. Based in Beirut —{" "}
          <b>building the proof, not borrowing it.</b>
        </p>

        <div className="ff-cta ff-rise" data-d="4">
          <button type="button" className="ff-btn" onClick={() => scrollToSection("projects")}>
            See the work <span className="arr" aria-hidden="true">→</span>
          </button>
          <button type="button" className="ff-ghost" onClick={() => scrollToSection("contact")}>
            Get in touch
          </button>
        </div>
      </div>

      <div className="ff-strip">
        <div className="ff-stripline">
          <div className="ff-meta ff-rise" data-d="4">
            Beirut · 26 &nbsp;&nbsp;<span className="hint">— move the cursor: feel the lag, then the commit</span>
          </div>
          <div className="ff-status ff-rise" data-d="4">
            <span className="dot" aria-hidden="true" /> Status — Building · 2026
          </div>
        </div>
      </div>
    </section>
  );
}
