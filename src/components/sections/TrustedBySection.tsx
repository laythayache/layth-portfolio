...
/* Clients are managed in the CMS (/admin → "Trusted by" tab) and stored in R2.
   This section fetches GET /api/clients at runtime. Until any exist — or if the
   API is unavailable (e.g. plain `vite` dev without Functions) — it shows neutral
   placeholder cells so the layout stays visible. */
...
export default function TrustedBySection() {
  ...
  // Pull the live client list from the CMS (R2); fall back to placeholders.
  useEffect(() => {
    let alive = true;
    fetch("/api/clients")
      ...
      >
        ...
        <div className="tb-grid">
          {cells.map((c, i) => {
            ...
            const cell = <CellInner c={c} />;
            return (
              <motion.div
                key={c.id ?? i}
                variants={SECTION.fadeUp}
                className={`tb-cell${filled ? "" : " is-empty"}`}
              >
                {c.href ? (
